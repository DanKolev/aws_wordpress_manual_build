...
The application media and UI store is local to an instance, scaling IN/OUT risks this media. In part 5, I am creating an EFS file system 
...
Customers are getting connected to an instance directly. There are no health checks, nor auto healing. The IP of the instance is hardcoded into the database. This is fixed in part 6, by adding an auto scaling group to provision and terminate instances automatically based on load on the system. Which also enabled me to create a new Paramater store value the elastic balancer's DNS name.

In Part 7, I am upgrading RDS to Aurora, which provides more durability, scalability, resiliency, and performance when compared to RDS.
